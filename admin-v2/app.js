...
(function () {
  ...
  var VIEW = 'cockpit';      // cockpit | cktaches | ckplanning | ckprojets | clients | client | chat
  ...
  function renderVisios() {
    ...
    if (!TRAMES_LOADED) {
      api('/api/call-trames').then(function (r) { return r.json(); }).then(function (d) {
        ...
        // ailleurs : elles ne sont pas touchées).
        // Retour au vouvoiement : le drapeau change de nom pour que le
        // remplacement rejoue une fois de plus.
        var etu = false; try { etu = localStorage.getItem('stb_erdyn_vous') === '1'; } catch (e) {}
        if (!etu) {
          var iEr = -1;
          for (var z = 0; z < srv.length; z++) { if (srv[z] && srv[z].id === 't_erdyn_serenity') { iEr = z; break; } }
          if (iEr >= 0) { srv[iEr] = erdynTrame(); changed = true; }
          try { localStorage.setItem('stb_erdyn_vous', '1'); } catch (e) {}
        }
  ...
  function erdynTrame() {
    var L = [
      ...
      "« Concernant les validations, est-ce que vous serez mon interlocutrice principale et est-ce que les différents retours seront centralisés de votre côté avant de m'être transmis ? »",
      "Si non :",
      ...
      "",
      "⑬ POUR TERMINER",
      "« Est-ce qu'il y a une attente, une contrainte ou un élément important qui n'apparaît pas forcément dans le cahier des charges et que vous souhaiteriez que je prenne en compte dans ma proposition ? »",
  ...
  /* ════════════════════════════════════════════════════════════════════════
     COCKPIT — le nouvel Accueil.

     Un écran de DÉCISION, pas un tableau d'indicateurs. Quatre niveaux, dans
     l'ordre où les questions se posent :
       1. Qu'est-ce qui compte maintenant ?   TON CAP
       2. À quoi ressemble ma journée ?       TA JOURNÉE
       3. Qu'est-ce qui n'est pas normal ?    À TON ATTENTION
       4. Est-ce que ma semaine tient ?       CETTE SEMAINE

     Aucune priorité Haute/Moyenne/Basse : tout se déduit des faits (créneau du
     jour, retard, ce que la tâche débloque, place restante avant l'échéance).
     Chaque recommandation porte donc sa raison.

     Règle de survie de ce bloc : quand le travail restant n'a jamais été dit,
     on répond « je ne sais pas » plutôt que zéro. Un cockpit qui invente des
     chiffres est pire que pas de cockpit.
     ════════════════════════════════════════════════════════════════════════ */

  var CKP = { pret: false, perso: [], plan: null, dash: null, cal: [], ordre: null };
  ...
  function ckpAuj() { return ckpIso(new Date()); }
  function ckpPlus(iso, n) { var d = ckpD(iso); d.setDate(d.getDate() + n); return ckpIso(d); }
  function ckpDow(iso) { return ((ckpD(iso).getDay() + 6) % 7) + 1; }
  function ckpOuvre(iso) { return ckpMinJour(iso) > 0; }
  function ckpJoursOuvres(a, b) {
    if (a === b) return 0;
    var sens = a < b ? 1 : -1, n = 0, cur = a, garde = 0;
    while (cur !== b && garde++ < 400) { cur = ckpPlus(cur, sens); if (ckpOuvre(cur)) n += sens; }
    return n;
  }
  ...
  function ckpQuand(iso) {
    ...
    if (iso === ckpPlus(auj, 1)) return 'demain';
    if (iso === ckpPlus(auj, -1)) return 'hier';
    var e = ckpJoursOuvres(auj, iso), d = ckpD(iso);
    if (e > 0 && e <= 4) return CKP_JOURS[d.getDay()];
    if (e > 4 && e <= 9) return CKP_JOURS[d.getDay()] + ' prochain';
    if (e < 0 && e >= -5) return CKP_JOURS[d.getDay()] + ' dernier';
    return 'le ' + (d.getDate() === 1 ? '1er' : d.getDate()) + ' ' + CKP_MOIS[d.getMonth()];
  }
  function ckpMaj(s) { return s ? s.charAt(0).toUpperCase() + s.slice(1) : s; }

  /* ── Les tâches, normalisées UNE fois ─────────────────────────────────── */
  /* Tâches clientes, tickets de maintenance et tâches Seed to Bloom vivent
     dans trois endroits différents. Le cockpit les lit sous une seule forme :
     sans ça, chaque section réinventerait sa façon de les comprendre. */

  function ckpNorm(t, src) {
    return {
      id: t.id, key: t.key || '', src: src,
      projet: t.project || (src === 'ticket' ? 'maintenance' : 'partner'),
      titre: t.title || 'Sans titre',
      qui: src === 'perso' ? 'Seed to Bloom' : (t.client || ''),
      ctx: src === 'perso' ? (t.clientName || '') : (t.projectLabel || ''),
      statut: t.status || 'todo',
      echeance: String(t.dueDate || '').slice(0, 10),
  ...
  function ckpRestant(t) {
    if (typeof t.restant === 'number') return t.restant;
    // Jamais dit : on se rabat sur l'estimation moins le réel, et seulement si
    // une estimation existe. Sinon on assume de ne pas savoir.
    if (t.estim > 0) return Math.max(0, t.estim - t.reel);
    return null;
  }
  function ckpFinCreneau(c) { return Math.min(1440, (c.start || 0) + (c.minutes || 0)); }
  function ckpCreneauPasse(c) {
    var auj = ckpAuj(), now = new Date().getHours() * 60 + new Date().getMinutes();
    return c.date < auj || (c.date === auj && ckpFinCreneau(c) <= now);
  }
  function ckpPlanifieFutur(t) {
    return (t.slots || []).reduce(function (s, c) { return s + (ckpCreneauPasse(c) ? 0 : (c.minutes || 0)); }, 0);
  }
  function ckpAPlanifier(t) {
    var r = ckpRestant(t);
    if (r === null) return null;
    return Math.max(0, r - ckpPlanifieFutur(t));
  }
  function ckpEnRetard(t) { return !!t.echeance && t.echeance < ckpAuj(); }
  function ckpPhraseTemps(t) {
    var b = [], r = ckpRestant(t), pf = ckpPlanifieFutur(t), ap = ckpAPlanifier(t);
    if (r === null) b.push('<b class="ck-inc">temps à estimer</b>');
    else if (r) b.push('<b>' + esc(ckpDuree(r)) + '</b> à faire');
    if (pf) b.push('<b>' + esc(ckpDuree(pf)) + '</b> planifiées');
    if (ap) b.push('<b class="ck-ap">' + esc(ckpDuree(ap)) + '</b> à planifier');
    if (t.reel) b.push(esc(ckpDuree(t.reel)) + ' passées');
    return b.join('<span class="ck-pt"></span>');
  }

  /* ── Capacité : une seule définition, marge retirée UNE fois ──────────── */

  function ckpMinJour(iso) {
  ...
  function ckpEnv(id) {
    var l = (CKP.plan && CKP.plan.enveloppes) || [];
    for (var i = 0; i < l.length; i++) if (l[i].id === id) return Math.max(0, l[i].minutes || 0);
    return 0;
  }
  function ckpMargeJour() {
    var n = ckpJoursSemaine().length || 5;
    return Math.round(ckpEnv('marge') / n);
  }
  function ckpMessages() { return (CKP.plan && CKP.plan.messages) || { heure: 860, duree: 0 }; }

  /* Tous les segments occupés d'une journée : créneaux de tâches, rendez-vous
     du calendrier, et le bloc de consultation des messages. On les FUSIONNE :
     une petite tâche posée dans le bloc messages ne doit pas être comptée deux
     fois. */
  function ckpSegments(iso) {
    var segs = [];
    ckpTaches().forEach(function (t) {
      (t.slots || []).forEach(function (c) {
        if (c.date === iso) segs.push([c.start || 0, ckpFinCreneau(c)]);
      });
    });
    (CKP.cal || []).forEach(function (e) {
      var iv = (typeof msEventInterval === 'function') ? msEventInterval(e) : null;
      var d = String(e.start || e.date || '').slice(0, 10);
      if (iv && d === iso) segs.push([iv.s, iv.en]);
    });
    var m = ckpMessages();
    if (m.duree > 0 && ckpMinJour(iso) > 0) segs.push([m.heure, m.heure + m.duree]);
    return segs.filter(function (s) { return s[1] > s[0]; }).sort(function (a, b) { return a[0] - b[0]; });
  }
  function ckpFusion(segs, depuis) {
    var total = 0, fin = -1;
    segs.forEach(function (s0) {
      var s = [Math.max(s0[0], depuis == null ? -1 : depuis), s0[1]];
      if (s[1] <= s[0]) return;
  ...
  function ckpCapaciteDu(iso) {
    var auj = ckpAuj();
    var maintenant = new Date().getHours() * 60 + new Date().getMinutes();
    var certaine = Math.max(0, ckpMinJour(iso) - ckpMargeJour());
    if (iso === auj) {
      // Le temps déjà écoulé n'est plus une capacité.
      var deb = Math.round((CKP.plan && CKP.plan.startHour != null ? CKP.plan.startHour : 9.5) * 60);
      certaine = Math.max(0, certaine - Math.max(0, Math.min(maintenant, 1440) - deb));
    }
    var engage = iso < auj ? 0 : ckpFusion(ckpSegments(iso), iso === auj ? maintenant : -1);
    return { date: iso, certaine: certaine, engagee: engage,
      libre: Math.max(0, certaine - engage), depassement: Math.max(0, engage - certaine),
      mobilisable: ckpMargeJour(), passe: iso < auj, auj: iso === auj };
  }
  function ckpLibreJusqua(iso) {
    return ckpJoursSemaine().filter(function (j) { return j >= ckpAuj() && j <= iso; })
      .reduce(function (s, j) { return s + ckpCapaciteDu(j).libre; }, 0);
  }

  /* ── Priorité : déduite des faits, avec sa raison ─────────────────────── */

  var CKP_SEUIL = 15;
  function ckpSignaux(t) {
    var s = [], auj = ckpAuj();
    var cren = (t.slots || []).filter(function (c) { return c.date === auj && !ckpCreneauPasse(c); })[0];
    if (ckpEnRetard(t)) {
      var j = Math.abs(ckpJoursOuvres(t.echeance, auj));
      s.push({ p: 60 + j * 10, texte: 'En retard depuis ' + ckpQuand(t.echeance) });
    }
    if (cren) s.push({ p: 50, texte: 'Prévue à ' + ckpHM(cren.start || 0), cren: cren });
    if (t.debloque) s.push({ p: 35, texte: 'Elle débloque ' + t.debloque });
    if (t.echeance === auj) s.push({ p: 30, texte: 'À rendre aujourd’hui' });
    else if (t.echeance && ckpJoursOuvres(auj, t.echeance) === 1) s.push({ p: 20, texte: 'À rendre demain' });
    var ap = ckpAPlanifier(t);
    if (ap && t.echeance && t.echeance >= auj) {
      var dispo = ckpLibreJusqua(t.echeance);
      if (ap > dispo) s.push({ p: 40, texte: 'Il n’y a plus assez de place avant ' + ckpQuand(t.echeance) });
      else if (ap > dispo * 0.6) s.push({ p: 18, texte: 'La place se réduit avant ' + ckpQuand(t.echeance) });
    }
    if (t.statut === 'review') s.push({ p: -40, texte: 'Chez la cliente' });
    return s.sort(function (a, b) { return b.p - a.p; });
  }
  function ckpScore(t) { return ckpSignaux(t).reduce(function (a, x) { return a + x.p; }, 0); }
  function ckpCap() {
    var forcé = CKP.ordre;
    var l = ckpTaches().filter(function (t) {
      if (t.statut === 'review' || t.statut === 'waiting_client') return false;
      var r = ckpRestant(t);
      return r === null || r >= CKP_SEUIL;
    }).map(function (t) { return { t: t, sc: ckpScore(t), sig: ckpSignaux(t) }; })
      .filter(function (x) { return x.sc > 0; })
      .sort(function (a, b) { return b.sc - a.sc; });
    if (forcé && forcé.length) {
      l.sort(function (a, b) {
        var ia = forcé.indexOf(a.t.id), ib = forcé.indexOf(b.t.id);
        if (ia < 0 && ib < 0) return b.sc - a.sc;
        if (ia < 0) return 1; if (ib < 0) return -1;
        return ia - ib;
      });
    }
    return l.slice(0, 3).map(function (x) {
      var second = x.sig[1] && x.sig[1].p >= 20 ? x.sig[1].texte : '';
      var cren = (x.t.slots || []).filter(function (c) { return c.date === ckpAuj() && !ckpCreneauPasse(c); })[0];
      return { t: x.t, raison: x.sig[0] ? x.sig[0].texte : '', raison2: second, cren: cren || null };
    });
  }

  /* ── À ton attention : ce qui n'est pas normal, et rien d'autre ───────── */

  function ckpAttention() {
    var capIds = ckpCap().map(function (c) { return c.t.id; });
    var out = [], auj = ckpAuj();

    // Du travail sans place. Plusieurs tâches = UNE situation, pas trois
    // alertes jumelles : ce qui compte, c'est le volume qui n'a nulle part
    // où aller.
    var sansPlace = ckpTaches().filter(function (t) {
      if (capIds.indexOf(t.id) >= 0 || !t.echeance) return false;
      var ap = ckpAPlanifier(t);
      if (!ap) return false;
      var lim = t.echeance < auj ? auj : t.echeance;
      return ckpJoursOuvres(auj, lim) <= 5;
    }).sort(function (a, b) { return a.echeance < b.echeance ? -1 : 1; });
    if (sansPlace.length) {
      var vol = sansPlace.reduce(function (s, t) { return s + ckpAPlanifier(t); }, 0);
      ...
      out.push({ g: ckpAPlanifier(pire) > dispo ? 90 : 58,
        ...
        texte: sansPlace.slice(0, 3).map(function (t) {
          return '« ' + t.titre + ' » ' + ckpDuree(ckpAPlanifier(t)) + ' avant ' + ckpQuand(t.echeance);
        }).join(' · ') + '. Il reste ' + ckpDuree(dispo) + ' de libre d’ici ' + ckpQuand(lim) + '.',
        t: sansPlace.length === 1 ? pire : null });
    }
    ...
    if (tard.length) {
      out.push({ g: 85, ton: 'alerte',
        ...
        t: tard.length === 1 ? tard[0] : null });
    }

    // Des tâches dont on ne sait pas combien de temps elles demandent : sans
    ...
    var attente = ckpTaches().filter(function (t) {
  ...
  /* ── Cette semaine : est-ce que mon travail rentre ? ──────────────────── */

  function ckpSemaine() {
    var jours = ckpJoursSemaine().map(ckpCapaciteDu);
    ...
    var fin = ckpJoursSemaine()[ckpJoursSemaine().length - 1] || ckpAuj();
    var aCaser = ckpTaches().filter(function (t) {
      return t.echeance && t.echeance <= fin && ckpAPlanifier(t) > 0;
    });
    var besoin = aCaser.reduce(function (s, t) { return s + ckpAPlanifier(t); }, 0);
    // Même filtre que « À ton attention » : ce qui attend la cliente n'est pas
    // du travail à estimer. Deux comptes différents pour la même chose
    // donneraient deux chiffres à l'écran.
    var inconnu = ckpTaches().filter(function (t) {
      return ckpRestant(t) === null && t.statut !== 'review';
    }).length;
    var verdict = 'confortable', cause = 'volume';
    if (besoin > libre) verdict = 'surcharge';
    else if (depassement > 0) { verdict = 'tendu'; cause = 'depassement'; }
    else if (besoin > libre * 0.7) verdict = 'tendu';
    return { jours: jours, libre: libre, besoin: besoin, verdict: verdict,
      cause: cause, inconnu: inconnu, depassement: depassement };
  }
  ...
  function ckpPasMaintenant(id) {
    var ids = ckpCap().map(function (c) { return c.t.id; });
    var i = ids.indexOf(id);
    if (i >= 0 && i < ids.length - 1) { ids.splice(i, 1); ids.push(id); }
    CKP.ordre = ids;
    renderCockpitBody();
  }
  function ckpOrdreSysteme() { CKP.ordre = null; renderCockpitBody(); }
  ...
  function ckpParseDuree(v) {
    ...
    if (/^\d+(?:\.\d+)?$/.test(s)) {
      var n = parseFloat(s);
      // Un nombre à virgule, ou petit, se dit en heures ; un entier en minutes.
      return Math.round(s.indexOf('.') >= 0 ? n * 60 : n);
    }
    return null;
  }
  function ckpUrl(t) {
  ...
  // ── Helpers planning (blocs iCloud figés + temps libre) ────────────────────
  // Couleur de catégorie (pastille) d'une tâche pour le planning.
  function msCatColor(cls) { return { 'wt-client': 'var(--glycine-700)', 'wt-crea': 'var(--gold-chip)', 'wt-studio': 'var(--glycine-900)', 'wt-admin': 'var(--terre-400)', 'wt-rdv': 'var(--terre)' }[cls] || 'var(--gold-chip)'; }
  // Intervalle horaire (en minutes depuis minuit) d'un événement iCloud daté.
  // null si journée entière ou sans heure.
  function msEventInterval(e) {
    if (!e || !e.start || e.allDay) return null;
    var d = new Date(e.start); if (isNaN(d)) return null;
    var s = d.getHours() * 60 + d.getMinutes(), en;
    if (e.end) { var d2 = new Date(e.end); en = isNaN(d2) ? s + 60 : (d2.getHours() * 60 + d2.getMinutes()); if (en <= s) en = s + 60; }
    else en = s + 60;
    return { s: s, en: en };
  }
  ...
  // Construit la « timeline » d'un jour. Les blocs iCloud (« blocs de temps »)
  // sont des CONTENANTS : on y dépose des tâches (repère pour poser au bon
  // endroit). Une tâche est rattachée à un bloc via son champ slot = heure de
  // début du bloc. Le temps libre entre les blocs reste dispo aussi.
  // Réutilisé par la vue Semaine (compact) et Aujourd'hui.
  function msBuildTimeline(diso, dow) {
    ...
    // Rattache les tâches aux blocs (par slot = heure de début), le reste = « libres ».
    var byBlock = {}; evs.forEach(function (ev) { byBlock[String(ev.start)] = []; });
    var loose = [];
    act.forEach(function (t) { var s = t.slot || ''; if (s !== '' && byBlock.hasOwnProperty(s)) byBlock[s].push(t); else loose.push(t); });
    var items = evs.map(function (ev) { return { type: 'block', start: ev.start, end: ev.end, e: ev.e, tasks: byBlock[String(ev.start)] || [] }; })
      .concat(zones.map(function (z) { return { type: 'free', start: z.start, end: z.end, label: z.label, tasks: [] }; }))
      .sort(function (a, b) { return a.start - b.start; });
    // Répartit les tâches « libres » (sans bloc) dans les zones libres.
    var freeItems = items.filter(function (it) { return it.type === 'free'; });
    var queue = loose.slice();
  ...
  function renderMaSemaineBody() {
    ...
    var rangeLbl = days[0].getDate() + ' → ' + days[4].getDate() + ' ' + MS_MONTHS[days[4].getMonth()];
    ...
    function advTaskLbl(t) { return '<b>' + esc(t.title) + '</b>' + (t.clientName ? ' · ' + esc(t.clientName) : '') + (t.estMinutes ? ' · ~' + msDur2(t.estMinutes) : ''); }
    ...
    if (overdueA.length) {
      ...
      focusMsg = billToday.length ? 'Aujourd\'hui, attaque le <b>facturable</b> en premier : ' + advTaskLbl(billToday[0]) + '.' : 'Aujourd\'hui, commence par la plus importante et garde ton élan.';
    ...
    var focusIc = focusTone === 'attn'
      ? '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 9v4M12 17h0M10.3 3.9 2.4 18a2 2 0 0 0 1.7 3h15.8a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/></svg>'
      : '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M12 3a6 6 0 0 0-4 10.5c.7.7 1 1.2 1 2.5h6c0-1.3.3-1.8 1-2.5A6 6 0 0 0 12 3zM9 20h6M10 22h4"/></svg>';
    var focus = '<div class="mss-focus' + (focusTone === 'attn' ? ' mss-focus--attn' : '') + '"><span class="mss-focus__ic">' + focusIc + '</span><div class="mss-focus__t">' + focusMsg + '</div></div>';
    // ── Barre d'outils : bascule Semaine / Aujourd'hui + navigation + capacité ──
    var seg = '<div class="mss-seg">' +
      '<button class="' + (MS_MODE !== 'day' ? 'on' : '') + '" onclick="ADM.msMode(\'week\')">Semaine</button>' +
      '<button class="' + (MS_MODE === 'day' ? 'on' : '') + '" onclick="ADM.msMode(\'day\')">Aujourd\'hui</button>' +
      '</div>';
    var nav = MS_MODE === 'day' ? '' : '<div class="mss-nav">' +
      '<button onclick="ADM.msWeek(-1)" title="Semaine précédente">←</button>' +
      '<button class="today" onclick="ADM.msWeek(0)" title="Cette semaine">Aujourd\'hui</button>' +
      '<button onclick="ADM.msWeek(1)" title="Semaine suivante">→</button></div>' +
      '<span class="mss-range">' + rangeLbl + '</span>';
    ...
    function chip(t) {
      var cls = msWt(t), late = (t.dueDate && t.dueDate.slice(0, 10) < todayIso) ? '<span class="mss-ptask__late">retard</span>' : '';
      var nm = esc(t.title) + (t.clientName ? ' · ' + esc(t.clientName) : '');
      return '<div class="mss-ptask" draggable="true" ondragstart="ADM.msDragStart(event,\'' + t.id + '\')" ondragend="ADM.msDragEnd()">' +
    ...
    function blockHtml(it, diso) {
      var e = it.e, vis = !!e.joinUrl, key = String(it.start);
      var ic = vis
        ? '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M23 7l-7 5 7 5V7zM14 5H3a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2z"/></svg>'
        : '<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M4 5h16v15H4zM4 9h16M8 3v4M16 3v4"/></svg>';
    ...
    // Bloc de temps iCloud = CONTENANT compact (vue Semaine). On y dépose des tâches.
    function blockWeek(it, diso) {
      var e = it.e, vis = !!e.joinUrl, key = String(it.start);
      var ic = vis
    ...
    if (MS_MODE === 'day') {
      ...
      var agenda = tl.items.map(function (it) {
        if (it.type === 'block') return blockHtml(it, diso);
      ...
      function dayCard(d) {
        ...
        var body = tl.items.map(function (it) {
          if (it.type === 'block') return blockWeek(it, diso);
          var dur = (it.start != null) ? (it.end - it.start) : 0;
          var lbl = (it.label ? esc(it.label) : 'Libre') + (dur ? ' · ' + msDur2(dur) : '');
          var tHtml = it.tasks.map(tblock).join('');
  ...
  function msMode(m) { MS_MODE = (m === 'day' ? 'day' : 'week'); if (m === 'day') MS_OFFSET = 0; renderMaSemaineBody(); }
  function msDaySel(i) { MS_DAYSEL = i; renderMaSemaineBody(); }
  // Persiste doDate (et éventuellement slot) pour une liste de tâches déjà mises à jour localement.
  function msPersistPlacement(list) {
    list.forEach(function (t) {
      var url = t._src === 'client' ? '/api/clients/' + t._key + '/tasks/' + t.id : '/api/admin/tasks/' + t.id;
      var body = t._src === 'client' ? { projectId: 'partner', doDate: t.doDate, slot: t.slot || '' } : { doDate: t.doDate, slot: t.slot || '' };
      jpost(url, body, 'PATCH').catch(function () {});
    });
  }
  ...
  // Un bloc iCloud « reçoit » une tâche si son titre contient le prénom de la
  // cliente (ex. tâche de « Marie Minchella » → bloc « Créneau Marie »).
  function msBlockMatch(t, title) {
    var hay = String(title || '').toLowerCase();
    var cn = String(t.clientName || '').trim().toLowerCase();
    if (cn) { var fn = cn.split(/\s+/)[0]; if (fn.length >= 3 && hay.indexOf(fn) >= 0) return true; }
    return false;
  }
  // Construit, pour une liste de jours, les blocs iCloud « à remplir » (non-visio)
  // avec leur capacité restante + le temps libre restant.
  function msDayTargets(days) {
    return days.map(function (d) {
      var diso = msIso(d), dow = ((d.getDay() + 6) % 7) + 1;
      var evs = msCalForDay(diso).map(function (e) { var iv = msEventInterval(e); return (iv && !e.joinUrl) ? { start: iv.s, end: iv.en, title: e.title || '' } : null; }).filter(Boolean);
      var blocks = evs.map(function (ev) {
        var used = MS_ALL.filter(function (t) { return !t.archived && t.status !== 'done' && (t.doDate || '').slice(0, 10) === diso && (t.slot || '') === String(ev.start); }).reduce(function (s, t) { return s + (t.estMinutes || 0); }, 0);
        return { key: String(ev.start), title: ev.title, cap: Math.max(0, (ev.end - ev.start) - used) };
      });
      return { diso: diso, blocks: blocks, free: msWindowFree(d) };
    });
  }
  // Cale les tâches du pool par ordre d'importance : d'abord dans le bloc du bon
  // projet (match nom), sinon dans un bloc libre, sinon dans le temps libre du
  // jour le moins chargé. Retourne {changed, placedCount, leftover}.
  function msFill(pool, targets) {
    var changed = [], placedCount = 0, leftover = 0;
    function assign(t, diso, slot) { if ((t.doDate || '').slice(0, 10) !== diso || (t.slot || '') !== slot) { t.doDate = diso; t.slot = slot; changed.push(t); } placedCount++; }
    pool.forEach(function (t) {
      var need = t.estMinutes || 30, best = null, bestCap = -1;
      // 1) bloc qui correspond au projet
      targets.forEach(function (di) { di.blocks.forEach(function (b) { if (b.cap >= need && msBlockMatch(t, b.title) && b.cap > bestCap) { bestCap = b.cap; best = { di: di, b: b }; } }); });
      // 2) sinon n'importe quel bloc avec de la place
      if (!best) { targets.forEach(function (di) { di.blocks.forEach(function (b) { if (b.cap >= need && b.cap > bestCap) { bestCap = b.cap; best = { di: di, b: b }; } }); }); }
      if (best) { best.b.cap -= need; assign(t, best.di.diso, best.b.key); return; }
      // 3) temps libre du jour le plus disponible
      var bd = null, bf = -1;
      targets.forEach(function (di) { if (di.free >= need && di.free > bf) { bf = di.free; bd = di; } });
      if (bd) { bd.free -= need; assign(t, bd.diso, ''); return; }
      leftover++;
    });
    return { changed: changed, placedCount: placedCount, leftover: leftover };
  }
  function msOrganizeWeek() {
    ...
    var r = msFill(pool, msDayTargets(days));
    renderMaSemaineBody();
    if (r.changed.length) msPersistPlacement(r.changed);
    msBadgesSoon();
    toast('Semaine organisée ✓ ' + r.placedCount + ' tâche' + (r.placedCount > 1 ? 's' : '') + ' rangée' + (r.placedCount > 1 ? 's' : '') + ' dans tes blocs (par importance)' + (r.leftover ? ' · ' + r.leftover + ' sans place' : ''));
  }
  // « Organise ma journée » : range les tâches à caler dans les blocs d'aujourd'hui
  // (bloc du bon projet en priorité), puis dans le temps libre.
  function msOrganizeDay() {
    var td = new Date();
    var pool = MS_ALL.filter(function (t) { if (t.archived || t.status === 'done' || t.mode === 'idee') return false; var dd = (t.doDate || '').slice(0, 10); return !dd || dd < msIso(td); }).sort(msPrioSort);
    if (!pool.length) { toast('Rien à caler aujourd\'hui'); return; }
    var r = msFill(pool, msDayTargets([td]));
    if (!r.placedCount) { toast('Pas de place dans tes blocs aujourd\'hui'); return; }
    renderMaSemaineBody();
    if (r.changed.length) msPersistPlacement(r.changed);
    msBadgesSoon();
    toast('Journée organisée ✓ ' + r.placedCount + ' tâche' + (r.placedCount > 1 ? 's' : '') + ' rangée' + (r.placedCount > 1 ? 's' : '') + (r.leftover ? ' · ' + r.leftover + ' un autre jour' : ''));
  }
  // Retirer une tâche du planning (glissée vers le backlog / rail) : jour + bloc effacés.
  function msUnplace(ev, elm) {
  ...
  // Survol d'un bloc / zone libre : on met en évidence CE conteneur (et on stoppe
  // la propagation pour ne pas aussi surligner le jour entier).
  function msSlotOver(ev, elm) { ev.preventDefault(); ev.stopPropagation(); if (elm && !elm.classList.contains('wcol--over')) elm.classList.add('wcol--over'); }
  // Drop sur le jour (zone hors bloc) → placé ce jour, sans bloc (slot vidé).
  function msDrop(ev, diso, elm) {
    ev.preventDefault(); ev.stopPropagation();
    if (elm) elm.classList.remove('wcol--over');
    var id = MS_DRAG || (ev.dataTransfer && ev.dataTransfer.getData('text/plain')); MS_DRAG = null;
    if (id) msPatch(id, { doDate: diso, slot: '' });
  }
  // Drop DANS un bloc de temps iCloud → rattaché à ce bloc (slot = heure de début).
  function msDropSlot(ev, diso, slot, elm) {
    ev.preventDefault(); ev.stopPropagation();
    if (elm) elm.classList.remove('wcol--over');
    var id = MS_DRAG || (ev.dataTransfer && ev.dataTransfer.getData('text/plain')); MS_DRAG = null;
  ...
  function partnerTasks(d) {
    ...
    // Tâches regroupées PAR STATUT (À faire / En cours / À valider / Terminé).
    var grid;
