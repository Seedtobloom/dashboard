        ...
        if (pc) {
            ...
            (pc.taches || []).forEach((t) => {
                ...
                if (t.status !== 'done' && (t.dueDate || t.status === 'review')) {
                    ...
                    const atts = (t.attachments || []).map((a) => ({ name: a.name || 'fichier', key: a.key || a.fileKey || '' })).filter((a) => a.key);
                    ...
                    try {
                        ...
                        if (o && o.link)
                            clientLink = String(o.link).slice(0, 500);
                        if (o && Array.isArray(o.files))
                            o.files.forEach((f) => { if (f && f.key)
                                beFiles.push({ name: String(f.name || 'fichier').slice(0, 120), key: String(f.key).slice(0, 300) }); });
                    }
                    catch (e) { /* ignore */ }
                ...
                if (t.clientNotif && !t.archived)
                    newTasks.push({ key: ci.key, client: who, id: t.id, title: t.title, content: t.content || '', dueDate: t.dueDate || '', attCount: (t.attachments || []).length, attachments: (t.attachments || []).map((a) => ({ name: a.name || 'fichier', key: a.key || a.fileKey || '' })).filter((a) => a.key), createdAt: t.createdAt || '' });
                ...
                if (t.clientCommentNotif && !t.archived) {
                    const cc = Array.isArray(t.comments) ? t.comments.filter((c) => c.author === 'client') : [];
...
function backupTime(o) {
    if (o.uploaded && !isNaN(o.uploaded.getTime()))
        return o.uploaded.getTime();
...
async function backupSnapshot(env) {
    ...
    for (let i = 1; i < withTime.length; i++) {
        if (withTime[i].t && withTime[i].t < cutoff)
            await env.R2_FILES.delete(withTime[i].key);
    }
