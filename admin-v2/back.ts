...
async function handleLogin(request: Request, env: Env): Promise<Response> {
  ...
  if (keyA !== auth.keyA || keyB !== auth.keyB) return json({ error: 'Clés invalides' }, 401);
...
async function handleUpload(request: Request, env: Env, key: string, data: AnyObj): Promise<Response> {
  ...
  if (asDeliverable) {
    ...
    if (container) {
      ...
      const existing = taskId ? container.livrables.find((l: AnyObj) => l.taskId === taskId && (l.status === 'refuse' || l.status === 'revision')) : null;
      if (existing) {
        existing.name = file.name; existing.fileKey = r2key; existing.status = 'a_valider'; existing.clientComment = ''; existing.validatedAt = null; existing.createdAt = nowIso();
...
async function handleDeliverablePatch(request: Request, env: Env, key: string, data: AnyObj, id: string): Promise<Response> {
  ...
  if (body.status) liv.status = body.status;
  await saveClient(env, key, data);
  return json(liv);
}
