    ...
    const collectLiv = (container: AnyObj | null, label: string, projectId: string) => {
      ...
      container.livrables.forEach((l: AnyObj) => {
        ...
        if (st === 'a_valider') {
          ...
          pendingValidation.push({ key: ci.key, client: who, project: projectId, projectLabel: label, id: l.id, name: l.name || '', createdAt: l.createdAt || null, taskId: l.taskId || null, taskTitle: l.taskTitle || '', timeSpentSeconds: lvSec });
          ...
          revisions.push({ key: ci.key, client: who, project: projectId, projectLabel: label, id: l.id, creationId: l.creationId || null, seenByAdmin: l.seenByAdmin === true, name: l.name || '', taskId: l.taskId || null, taskTitle: l.taskTitle || '', comment: l.clientComment || '', wishDate: l.clientWishDate || '', at: l.validatedAt || null, attachments: files, clientLink, reviewLink: l.reviewLink || '' });
    ...
    if (pc) {
      ...
      (pc.taches || []).forEach((t: AnyObj) => {
        ...
        if (t.stage === 'inbox' && !t.archived) {
          ...
          inbox.push({ key: ci.key, client: who, id: t.id, title: t.title || '', content: t.content || '', blocks: Array.isArray(t.blocks) ? t.blocks : [], table: (t.table && typeof t.table === 'object') ? t.table : null, urgency: t.urgency || 'normal', dueDate: t.dueDate || '', createdAt: t.createdAt || '', demandeType: t.demandeType || '', missionType: t.missionType || (t.properties && t.properties.p_typemission) || '', precisions: (t.properties && t.properties.p_precisions) || '', estMinutes: typeof t.estMinutes === 'number' ? t.estMinutes : (Number(t.properties && t.properties.p_estime) || 0), exception: (t.properties && t.properties.p_exception) || '', besoins: (() => { try { const b = JSON.parse((t.properties && t.properties.p_besoins) || '[]'); return Array.isArray(b) ? b : []; } catch (e) { return []; } })(), attachments: atts, clientLink, forfaitRemaining: fs.remaining, forfaitConfigured: fs.configured, monthCount, avgMinutes });
