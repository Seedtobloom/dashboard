  ...
  function stbBlockRow(pid, taskId, b, i, n, num){
    ...
    if (b.type === 'sep') {
      ...
      var tbody = b.rows.slice(1).map(function(row, ri){
        ...
        }).join('') + '<td style="border:none;width:20px;text-align:center;vertical-align:top"><button onclick="window.stbTableDelRow(\''+pid+'\',\''+taskId+'\',\''+b.id+'\','+rr+')" title="Supprimer la ligne" style="border:none;background:none;color:#7a5540;cursor:pointer;font-size:11px;opacity:0.45;margin-top:8px">✕</button></td></tr>';
      }).join('');
      inner = '<div style="flex:1;min-width:0;overflow-x:auto"><table style="border-collapse:collapse;width:100%;background:#fff;border-radius:8px"><tbody>'+thead+tbody+'</tbody></table>'+
        '<div style="display:flex;gap:7px;margin-top:9px;flex-wrap:wrap;align-items:center">'+
          '<button class="stb-tbtn" onclick="window.stbTableAddRow('+dA+')">Ajouter une ligne</button>'+
          '<button class="stb-tbtn" onclick="window.stbTableAddCol('+dA+')">Ajouter une colonne</button>'+
