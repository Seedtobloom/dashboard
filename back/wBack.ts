...
import { requireAdminAuth, verifyClientToken } from './auth';
...
export async function handleRequest(request: Request, env: Env): Promise<Response> {
  ...
  // ── CLIENT PORTAL /p/{token} — serve client.html SPA ───────────────────────
...
// ── Client portal — SSR: verifies token, returns client.html with token in meta ──

async function handleClientPortal(_request: Request, env: Env, url: URL): Promise<Response> {
  ...
  // Redirect to the front worker's client portal with the token
  // The front worker serves client.html as an SPA; the token is in the URL path.
  // Since the back worker owns /p/{token}, we serve a minimal redirect HTML
  // that points the browser to the same URL (already here) but with the SPA loaded.
  // Actually: the back worker IS the one serving this. We inline client portal HTML here.
  // The client.html SPA reads the token from window.location.pathname and calls
  // /api/client/{token} to get data. We just serve the SPA shell.
...
function clientPortalShell(token: string): string {
  return `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="utf-8">
...
(function() {
  ...
  function renderMessages(messages) {
    if (!messages.length) {
      return '<p class="empty-state">Pas encore de messages.<br>N\\'hésitez pas à m\\'écrire !</p>';
    }
  ...
  function renderSteps(steps) {
    if (!steps.length) return '<p class="empty-state">Les étapes seront bientôt définies.</p>';
  ...
  function renderFiles(files) {
    if (!files.length) return '<p class="empty-state">Aucun fichier partagé.</p>';
    const byCategory = {
      ...
      '<header class="page-header">' +
        '<div class="page-header__inner">' +
          ...
          '<div class="cindy-banner">' +
            ...
            '<div><strong>Cindy</strong> · Votre interlocutrice<br>' +
            '<span style="opacity:0.7;font-size:12px">Seed to Bloom · seedtobloom.fr</span></div>' +
          '</div>' +
          '<nav class="nav-tabs">' +
        ...
        (project.meetingLink ?
          '<section id="visio" class="section">' +
            ...
            '<div class="meeting-card">' +
              ...
              '<div class="meeting-info"><h4>Rejoindre la réunion</h4><p>Cliquez ci-dessous pour accéder à la visioconférence</p></div>' +
  ...
  fetch(API_BASE)
    ...
    .catch(() => {
      document.getElementById('app').innerHTML =
        ...
        '<p style="color:#666;line-height:1.7;font-size:15px">Le lien a expiré ou a été révoqué.<br><br>Contactez <a href="mailto:hello@seedtobloom.fr" style="color:#7fa688">Cindy</a> pour obtenir un nouveau lien.</p>' +
        '</div></div>';
    });
})();
</script>
