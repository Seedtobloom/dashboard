...
(function() {
  ...
  function renderApp(data) {
    ...
    const actionStep = project.steps.find(function(s) { return s.status === 'waiting_client'; });
    ...
    const messagesHtml = messages.length ? messages.map(function(m) {
    ...
    const practicalHtml = project.practicalInfo.sections.map(function(s) {
      return '<details class="practical-section"><summary>' + esc(s.title) + '</summary>' +
    ...
    function categoryHtml(label, items) {
      ...
      return '<div class="files-category"><h4>' + label + '</h4>' +
        items.map(function(f) {
          return '<a class="file-item" href="/api/projects/' + project.id + '/files/' + encodeURIComponent(f.key) + '/download" target="_blank">' +
  ...
  fetch(API_BASE)
    ...
    .then(function(data) { renderApp(data); })
